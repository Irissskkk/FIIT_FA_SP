...
fraction fraction::root(size_t degree, fraction const &epsilon) const {
    ...
    fraction res = *this / fraction(degree, 1);
    fraction prev_res;
    do {
        prev_res = res;
        fraction power = res.pow(degree - 1);
        if (power._numerator == 0) throw std::logic_error("Division by zero in root calculation");

        res = (fraction(degree - 1, 1) * res + *this / power) / fraction(degree, 1);
    } while ((res - prev_res > epsilon) || (prev_res - res > epsilon));

    if (_numerator < 0 && degree % 2 == 1) res = -res;
