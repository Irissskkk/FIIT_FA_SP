...
class AVL_tree final : public binary_search_tree<tkey, tvalue, compare, __detail::AVL_TAG> {
...
namespace __detail {
    ...
    template<typename tkey, typename tvalue, typename compare>
    void bst_impl<tkey, tvalue, compare, AVL_TAG>::post_insert(binary_search_tree<tkey, tvalue, compare, AVL_TAG>& cont,
        ...
        while (current) {
            ...
            node_type*& subtree_ref = (!current->parent) ? cont._root
                                                         : (current->parent->left_subtree == current
                                                            ? current->parent->left_subtree
                                                            : current->parent->right_subtree);

            if (balance > 1) {// Big right
                auto* right = static_cast<avl_node*>(current->right_subtree);
                if (right && right->get_balance() < 0) {// Right-Left
                    node_type*& right_ref = (right->parent->left_subtree == right) ? right->parent->left_subtree
            ...
            } else if (balance < -1) {// Big left
                auto* left = static_cast<avl_node*>(current->left_subtree);
                if (left && left->get_balance() > 0) {// Left-Right
    ...
    void bst_impl<tkey, tvalue, compare, AVL_TAG>::erase(
        ...
        if (!node->left_subtree && !node->right_subtree) {
            ...
            predecessor->left_subtree = (node->left_subtree == predecessor) ? predecessor->left_subtree : node->left_subtree;
            if (predecessor->left_subtree) {
            ...
            if (node->parent) {
                if (node->parent->left_subtree == node) node->parent->left_subtree = predecessor;
                else node->parent->right_subtree = predecessor;
            } else {
        ...
        if (balance_start) {
            ...
            while (current) {
                ...
                if (balance > 1) {// Big right
                    ...
                    auto* left = static_cast<avl_node*>(current->left_subtree);
                    if (left && left->get_balance() > 0) {// Left-Right
                        node_type*& left_ref = (left->parent->left_subtree == left) ? left->parent->left_subtree
                                                                                    : left->parent->right_subtree;
                        cont.small_left_rotation(left_ref);
                        left->recalculate_height();
                    }
...
template<typename tkey, typename tvalue, compator<tkey> compare>
template<input_iterator_for_pair<tkey, tvalue> iterator>
AVL_tree<tkey, tvalue, compare>::AVL_tree(iterator begin, iterator end, const compare& cmp,
                                          pp_allocator<value_type> alloc,
...
template<typename tkey, typename tvalue, compator<tkey> compare>
template<std::ranges::input_range Range>
AVL_tree<tkey, tvalue, compare>::AVL_tree(Range&& range,
                                          const compare& cmp,
                                          pp_allocator<value_type> alloc,
